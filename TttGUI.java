...
public class TttGUI extends BorderPane {
	...
	private class TttGrid extends GridPane  {
		...
		private ArrayList<Button> correctButtons = new ArrayList<Button>();
		...
		private EventHandler<MouseEvent> click = mouseEvent -> {
			...
			if(isFinished) {
			    ...
			    setEffectGroup(effectAll,correctButtons);
		...
		private void setEffectGroup(ParallelTransition pt,ArrayList<Button> correctButtons) {
			
			for(int i = 0; i < 3; i++) {
				for(int y = 0; y <3; y++) {
					if(!(buttons[i][y].equals(correctButtons.get(i)))) {
						 
						  
						  FadeTransition individualEffects = new FadeTransition();
						  individualEffects.setNode(buttons[i][y]);
						  individualEffects.setDuration(new Duration(1000));
						  individualEffects.setFromValue(1.0);
						  individualEffects.setToValue(0.0);
						  individualEffects.setCycleCount(6);
		...
		private boolean checkRows(TttCell cell,int xPos, int yPos) {
			...
			for(int j = 0; j < 3; j++) {
				if(cell.equals(cells[xPos][j]) && counter != 3) {
					counter++;
					s+="(" + xPos + ", " + j + "), ";
					correctButtons.add(buttons[xPos][j]);
		...
		private boolean checkColumns(TttCell cell,int xPos, int yPos) {
			...
			for(int j = 0; j < 3; j++) {
				if(cell.equals(cells[j][yPos]) && counter != 3) {
					counter++;
					correctButtons.add(buttons[j][yPos]);
				}
				else 
					counter = 0;
